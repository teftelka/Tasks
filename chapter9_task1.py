...
class BJ_Player(BJ_Hand):
    def is_hitting(self):
        response = games.ask_yes_no("\n" + self.name + ", will you take next card?(Y/N): ")
        return response == "y"
    def bust(self):
...
def main(): #ОТЭТА ФУНКЦИЯ
    print("\t\tWelcome to Black Jack!\n")
    names = []
    number = games.ask_number("How many people will play? (1 - 7): ", low = 1, high = 8)
    for i in range(number):
        name = input("Your name: ")
        while name == "": #и в этом условии что-то не так
